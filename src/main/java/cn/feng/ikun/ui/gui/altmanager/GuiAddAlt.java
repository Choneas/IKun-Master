...
public class GuiAddAlt extends GuiScreen {
    ...
    public void drawScreen(int i, int j, float f) {
        ...
        Client.instance.fontLoaders.get("baloo", 23).drawCenteredString("Add Alt", this.width / 2, 20, -1);
        this.username.drawTextBox();
        this.password.drawTextBox();
        this.combined.drawTextBox();
        if (this.username.getText().isEmpty()) {
            Client.instance.fontLoaders.get("baloo", 23).drawStringWithShadow("Username / E-Mail", this.width / 2 - 96, 66.0f, -7829368);
        }
        if (this.password.getText().isEmpty()) {
            Client.instance.fontLoaders.get("baloo", 23).drawStringWithShadow("Password", this.width / 2 - 96, 106.0f, -7829368);
        }
        if (this.combined.getText().isEmpty()) {
            Client.instance.fontLoaders.get("baloo", 23).drawStringWithShadow("Email:Password", this.width / 2 - 96, 146.0f, -7829368);
        }
        Client.instance.fontLoaders.get("baloo", 23).drawCenteredString(this.status, this.width / 2, 30, -1);
    ...
    public void initGui() {
        ...
        this.combined = new GuiTextField(this.eventButton, mc.fontRendererObj, this.width / 2 - 100, 140, 200, 20);
    ...
    private class AddAltThread extends Thread {
        ...
        private final void checkAndAddAlt(String username, String password) {
            ...
            try {
                auth.logIn();
                Client.instance.altManager.alts.add(new Alt(username, password));
        ...
        public void run() {
            if (this.password.equals("")) {
                Client.instance.altManager.alts.add(new Alt(this.username, ""));
                Client.instance.configManager.save();
