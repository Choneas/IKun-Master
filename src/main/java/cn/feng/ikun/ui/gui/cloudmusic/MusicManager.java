...
//	public ScrollingText artistsScroll;
    ...
    public MusicManager() {
        ...
        circleImage = new File(Minecraft.getMinecraft().mcDataDir.toString() + File.separator + Client.instance.name + File.separator + "circleImage");
        ...
        if (!mp3File.exists() && !flacFile.exists()) {
            ...
            loadingThread = new Thread(() -> {
                try {
                    ...
                    LogUtils.print("An error occurred while caching the music, possibly because the song has been removed or requires a VIP!");
                    if (mp3File.exists())
                        mp3File.delete();
