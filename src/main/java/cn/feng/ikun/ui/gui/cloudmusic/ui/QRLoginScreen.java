...
public class QRLoginScreen extends GuiScreen {
    ...
    public String state = "Waiting for user action...";
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        String text = "Use QR code to log in to NetEase Cloud Music";
        Client.instance.fontLoaders.wqy16.drawString(text, res.getScaledWidth() / 2 - (Client.instance.fontLoaders.wqy16.getStringWidth(text) / 2), 30, 0xFFFFFFFF);
        Client.instance.fontLoaders.wqy16.drawString(state, res.getScaledWidth() / 2 - (Client.instance.fontLoaders.wqy16.getStringWidth(state.replaceAll("\247.", "")) / 2), 50, 0xFFFFFFFF);

        if (this.loginProcessThread != null) {
            HanabiRenderUtils.drawImage(new ResourceLocation("cloudMusicCache/qrcode"), res.getScaledWidth() / 2 - 64, 70, 128, 128, 1f);
        }
