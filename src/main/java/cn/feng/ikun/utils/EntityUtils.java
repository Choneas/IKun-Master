...
import cn.feng.ikun.module.modules.player.Teams;
...
public class EntityUtils {
    ...
    public static boolean isSelected(final Entity entity, final boolean canAttackCheck) {
        if (entity instanceof EntityLivingBase && entity.isEntityAlive() && entity != mc.thePlayer) {
                ...
                if (canAttackCheck) {
                    if (((AntiBots) Client.instance.moduleManager.getModule(AntiBots.class)).isNPC(entityPlayer))
                        return false;

                    if (Client.instance.friendManager.isFriend(entityPlayer.getName()))
                        return false;

                    Teams t = (Teams) Client.instance.moduleManager.getModule(Teams.class);

                    if (t.isToggle() && t.isInYourTeam(entityPlayer))
                        return false;

                    if (entityPlayer instanceof EntityPlayer && ((EntityPlayer) entityPlayer).isSpectator())
                        return false;

                    final Teams teams = (Teams) Client.instance.moduleManager.getModule(Teams.class);
                    return !teams.isToggle() || !teams.isInYourTeam(entityPlayer);
                }
    ...
    public static boolean isValid(final EntityLivingBase entity) {
        return entity != null && entity != mc.thePlayer && entity.getDistanceToEntity(mc.thePlayer) <= 10000 && entity.isEntityAlive() && (!entity.isInvisible() || entity.getTotalArmorValue() != 0);
    }
