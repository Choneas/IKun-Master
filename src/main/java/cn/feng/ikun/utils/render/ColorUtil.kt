...
object ColorUtil {
    ...
    fun LiquidSlowly(time: Long, count: Int, qd: Float, sq: Float): Color? {
        val color = Color(Color.HSBtoRGB((time.toFloat() + count * -3000000f) / 2 / 1.0E9f, 0.56F, sq))
    ...
    fun rainbow3(offset: Long, rainbowSpeed: Float, rainbowBright: Float): Color {
        val currentColor =
            Color(Color.HSBtoRGB((System.nanoTime() + offset) / 10000000000F % 1, rainbowSpeed, rainbowBright))
        return Color(
            currentColor.red.toFloat() / 255.0f * 1.0f,
            currentColor.green.toFloat() / 255.0f * 1.0f,
            currentColor.blue.toFloat() / 255.0f * 1.0f,
            currentColor.alpha.toFloat() / 255.0f
        )
    }
    ...
    @JvmStatic
    fun fade1(color: Color, index: Int, count: Int): Color {
        val hsb = FloatArray(3)
        Color.RGBtoHSB(color.red, color.green, color.blue, hsb)
        var brightness =
            abs(((System.currentTimeMillis() % 2000L).toFloat() / 1000.0f + index.toFloat() / count.toFloat() * 2.0f) % 2.0f - 1.0f)
