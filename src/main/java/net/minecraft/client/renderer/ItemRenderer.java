...
import cn.feng.ikun.module.modules.render.BlockAnimation;
...
public class ItemRenderer {
    ...
    public void renderBlock(float swingProgress, float equippedProgress) {
        ...
        // 检测动画是否开启
        if (!Client.instance.moduleManager.getModule(BlockAnimation.class).isToggle()) {
    ...
    /* 这是Choneas写的大风车 然后死了*/
    int renderBlockingAngle = -180;
    private void renderBigWindmill() {
        GlStateManager.translate(-0.5F, 0.2F, 0.0F);
        GlStateManager.rotate(30.0F, 0.0F, 1.0F, 0.0F);
        GlStateManager.rotate(renderBlockingAngle, 1.0F, 0.0F, 0.0F);
        if (renderBlockingAngle >= 180) {
            renderBlockingAngle = -180;
        }
        renderBlockingAngle += 10;
        GlStateManager.rotate(60.0F, 0.0F, 1.0F, 0.0F);
    }
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            if (this.itemToRender != null) {
                ...
                } else if ((BlockAnimation.everything.getValue() && mc.gameSettings.keyBindUseItem.pressed) || (mc.gameSettings.keyBindUseItem.pressed && mc.thePlayer.getHeldItem() != null && mc.thePlayer.getHeldItem().getItem() instanceof ItemSword)) {
                    enumaction = EnumAction.BLOCK;
                    renderBlock(swingProgress, equippedProgress);
                } else {
                    if (!Client.instance.moduleManager.getModule(BlockAnimation.class).isToggle() || !BlockAnimation.swingAnimation.getValue()) {
                        this.doItemUsedTransformations(swingProgress);
                    }
                ...
                if (enumaction == EnumAction.NONE && Client.instance.moduleManager.getModule(BlockAnimation.class).isToggle()) {
    ...
    public void renderOverlays(float partialTicks) {
        ...
        if (!this.mc.thePlayer.isSpectator()) {
            if (this.mc.thePlayer.isInsideOfMaterial(Material.water)
                    ...
                    this.mc.thePlayer, Float.valueOf(partialTicks))) {
